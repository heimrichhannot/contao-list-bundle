...
$GLOBALS['TL_DCA']['tl_list_config_element'] = [
    'config'   => [
        ...
        'onload_callback' => [
            ['tl_list_config_element', 'checkPermission'],
        ],
        'onsubmit_callback' => [
...
class tl_list_config_element extends \Contao\Backend
{
    ...
    public function checkPermission()
    {
        ...
        switch (\Contao\Input::get('act'))
        {
            ...
            {
                throw new \Contao\CoreBundle\Exception\AccessDeniedException('Not enough permissions to create list_config_element items in list_config_element archive ID ' . \Contao\Input::get('pid') . '.');
            }
            break;
            ...
            {
                throw new \Contao\CoreBundle\Exception\AccessDeniedException('Not enough permissions to ' . \Contao\Input::get('act') . ' list_config_element item ID ' . $id . ' to list_config_element archive ID ' . \Contao\Input::get('pid') . '.');
            }
            ...
            case 'toggle':
            case 'feature':
            $objArchive = $database->prepare("SELECT pid FROM tl_list_config_element WHERE id=?")
            ->limit(1)
            ->execute($id);
            ...
            {
                throw new \Contao\CoreBundle\Exception\AccessDeniedException('Not enough permissions to ' . \Contao\Input::get('act') . ' list_config_element item ID ' . $id . ' of list_config_element archive ID ' . $objArchive->pid . '.');
            }
            break;
            ...
            if (!in_array($id, $root))
            {
                throw new \Contao\CoreBundle\Exception\AccessDeniedException('Not enough permissions to access list_config_element archive ID ' . $id . '.');
            }

            $objArchive = $database->prepare("SELECT id FROM tl_list_config_element WHERE pid=?")
            ->execute($id);
